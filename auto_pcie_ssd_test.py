...
if enable == "y":
    ...
    print("**********  " + time.strftime('%Y-%m-%d %H:%M:%S',begin_time) + "  start PCIE_SSD test  **********", sep="")
    ...
    #创建对应的日志目录 
    log_directory_name = "pcie_ssd_log"
    log_gen.mkdir(log_directory_name)  #调用函数

    #创建日志文件
    log_file_path = str(log_directory_name)+r"/"+"pcie_ssd" + "_"+str(time.strftime('%y%m%d_%H%M%S',begin_time))+".txt"
    ...
    if is_exist_device:
        ...
        print("PCIE_SSD device " + sddevice + " exists\n")
    ...
    print("\n**********  " + time.strftime('%Y-%m-%d %H:%M:%S',end_time) + "  PCIE_SSD test finish  **********", sep="")

    #写入结束时间
    log_file.write("\n" + "***************finish time: " + str(time.strftime('%y-%m-%d_%H:%M:%S',end_time)) + "***************")
    log_file.close()
else:
    print("PCIE_SSD not tested!")